(function() {
  ...
  function initTOC() {
    ...
    if (!contentEl) { tocContainer.style.display = 'none'; return; }
    ...
    headings.forEach(function(heading) {
      ...
      link.className = 'text-gray-500 dark:text-gray-400 hover:text-gray-800 dark:hover:text-gray-200 transition-colors';
    ...
    function setActive(link) {
      tocLinks.forEach(function(l) {
        l.classList.remove('font-semibold', 'text-gray-800', 'dark:text-gray-200');
      });
      if (link) link.classList.add.apply(link.classList, activeClass.split(' '));
    }
    ...
    var observer = new IntersectionObserver(function(entries) {
      ...
      if (visible.length > 0) {
        var first = visible[0];
        var link = document.querySelector('#table-of-contents a[href="#' + first.id + '"]');
        setActive(link);
      } else {
        var active = null;
        ...
        if (active) {
          var link = document.querySelector('#table-of-contents a[href="#' + active.id + '"]');
          setActive(link);
        } else {
